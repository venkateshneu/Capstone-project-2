...
class DataTransformation:
    ...
    def imbalance_data_cleaning(self):
        try:
            ...
            print(f"Using ID for dropping column: {self.data_transformation_config.ID}")
    ...
    def raw_data_cleaning(self):
        try:
            ...
            # Replace the value of 0 to 1 using .loc[]
            raw_data.loc[:, self.data_transformation_config.CLASS].replace({0: 1}, inplace=True)

            # Replace the value of 2 to 0 using .loc[]
            raw_data.loc[:, self.data_transformation_config.CLASS].replace({2: 0}, inplace=True)
    ...
    def concat_dataframe(self):
        try:
            ...
            # Concatenate the cleaned raw and imbalanced data
            frame = [self.raw_data_cleaning(), self.imbalance_data_cleaning()]
            df = pd.concat(frame)

            # Check the type of the `tweet` column to ensure it contains only strings
            assert df[self.data_transformation_config.TWEET].apply(lambda x: isinstance(x, str)).all(), "Non-string data found in `tweet` column"

            logging.info(f"Concatenated dataframe first few rows:\n{df.head()}")
            logging.info(f"Tweet column data types:\n{df[self.data_transformation_config.TWEET].apply(type).value_counts()}")

            # Log if there are any null values in the tweet column
            if df[self.data_transformation_config.TWEET].isnull().any():
                logging.warning("Null values found in `tweet` column")

            logging.info(f"Returned the concatenated dataframe with shape {df.shape}")
            return df
    ...
    def concat_data_cleaning(self, words):
        try:
            ...
            # Original words before cleaning
            logging.info(f"Original tweet: {words}")
            ...
            # Cleaned words
            logging.info(f"Cleaned tweet: {words}")
    ...
    def initiate_data_transformation(self) -> DataTransformationArtifacts:
        try:
            ...
            logging.info(f"Final transformed data saved to {self.data_transformation_config.TRANSFORMED_FILE_PATH}")
            logging.info(f"Final dataframe shape: {df.shape}")
            logging.info(f"Final dataframe first few rows:\n{df.head()}")
